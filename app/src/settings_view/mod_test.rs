...
fn collapsed_umbrella_is_a_single_nav_stop() {
    ...
    // Expect: <Agents umbrella>, BillingAndUsage, <Code umbrella>,
...
fn expanded_umbrella_produces_section_stop_per_subpage() {
    ...
    // ThirdPartyCLIAgents, BillingAndUsage, <Code umbrella>, <Cloud platform
    // umbrella>, Teams.
...
#[test]
fn arrow_down_from_teams_wraps_to_collapsed_agents_first_subpage() {
    let nav_items = realistic_nav_items();
    let stops = build_nav_stops(&nav_items, |_| true);

    // Pressing Down from the last top-level page wraps to the collapsed Agents
    // umbrella and selects WarpAgent.
...
fn arrow_down_collapsed_umbrella_respects_search_filter() {
    ...
    // From Teams, Down wraps to AgentMCPServers (first visible subpage of the
    // still-collapsed Agents umbrella), not to WarpAgent / AgentProfiles.
    let next = simulate_cycle(
        &nav_items,
        &stops,
        SettingsSection::Teams,
