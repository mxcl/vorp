...
#[test]
fn test_open_file_markdown_routes_to_session() {
    let dir = tempfile::tempdir().unwrap();
...
fn test_open_file_rust_source_routes_to_session() {
    let dir = tempfile::tempdir().unwrap();
...
#[test]
#[cfg(unix)]
fn test_open_file_non_runnable_shebang_routes_to_session() {
    // Terminal-only mode routes all file URLs into a terminal session instead
    // of opening Warp's editor or notebook views.
    use std::os::unix::fs::PermissionsExt;
    let dir = tempfile::tempdir().unwrap();
